...
class DecisionTree(object):
    ...
    def fit(self, X, Y):
        ...
        data_ = np.hstack((X, Y))
        data_.tolist()
        self.trees = build_tree(data_, current_depth=0, max_depth=self.max_depth)

    def predict(self, X):
        """
        :param X: 2 dimensional python list or numpy 2 dimensional arrayreturn: Y - 1 dimension python list with labels
        """
        y = []
